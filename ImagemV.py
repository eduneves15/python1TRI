...
class ImageBrowserApp:
    def __init__(self, root):
        ...
        self.image_paths = []#Caminho da imagem
        self.current_image_index = 0

        self.image_label = tk.Label(root)
        self.image_label.pack(padx=10, pady=10)

        browse_button = tk.Button(root, text="Selecionar Diretório", command=self.browse_directory) #Abre uma pasta para seleção.
        ...
        def browse_directory(self): #Abre diretório de pesquisa.
            directory_path = filedialog.askdirectory()
            if directory_path:
                self.image_paths = [
                    os.path.join(directory_path, file)
                    for file in os.listdir(directory_path)
                    if file.lower().endswith(('.png', '.jpg', '.jpeg', '.gif')) #Abre um LEK de tipos de imagens para selecionar.
                ]
                if self.image_paths:
                    self.current_image_index = 0
...
def show_image(self, index):
    ...
    try:
        ...
        image = image.resize((300,300))  #É o tamanho da imagem.
        photo = ImageTk.PhotoImage(image)
        self.image_label.config(image=photo)
        self.image_label.image = photo
    except Exception as e:
        self.image_label.config(text=f"Erro ao abrir imagem: {str(e)}") #Mostra que deu erro ao abrir a imagem.
        ...
        def show_next_image(self): #Troca a imagem para a próxima.
